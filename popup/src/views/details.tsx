...
import { Block, Bold, Button, ButtonInline, Callout, Card, Flex, Footer, List, ListItem, Metric, ProgressBar, Text, ColGrid, Title, Badge } from "@tremor/react";
import GandalfContext from "../context/gandalf.context";
import React from "react";
import { useNavigate, useParams } from "react-router-dom";
import { ColorForMark, getSingleCompetence, isFailure, isSuccess } from "../services/gandalf.service";
...
const Details: React.FC = memo(() => {
  ...
  const competence = React.useMemo(() => getSingleCompetence(data, code), [data]);
  ...
  const txt = isSuccess(competence) ? "acquis" : isFailure(competence) ? "échoué" : "non évaluée";
  return <>
      ...
      <Card maxWidth="max-w-xl" marginTop="mt-4">
        ...
        <Callout title={"Compétence " + txt + (txt !== "non évaluée" ? "e" : "")} text={txt == "non évaluée" ? `Cette compétence n'a pas encore été évaluée` : `Vous avez ${txt} la compétence avec le status "${competence.status}"`} color={color} marginTop="mt-6" />
        <ProgressBar percentageValue={succeeded.length * 100 / (totalExpectations || 1)} color={color} marginTop="mt-6" />
        <Flex marginTop="mt-4">
          <Block>
            <Text>Succès</Text>
        ...
        <List marginTop="mt-1">
          {[...succeeded, ...failed].map(item => <ListItem key={item.project}>
